...
def _scoring_node(state: GraphState) -> GraphState:
    leo_state = scoring_agent.run(state["leo_state"])
    return {"leo_state": leo_state}
...
def run_graph(url: str) -> LeoState:
    """Execute the Leo pipeline for a URL and return the final state."""
    graph = get_compiled_graph()
    initial_state: GraphState = {"leo_state": LeoState(url=url)}
